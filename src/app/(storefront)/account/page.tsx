...
export default async function AccountOverviewPage() {
  ...
  return (
    <div>
      ...
      <div className="mt-4 rounded-lg border border-ink-200 p-5">
        <p className="text-sm text-ink-500">Name</p>
        <p className="font-medium text-ink-800">{session.name}</p>
        <p className="mt-3 text-sm text-ink-500">Email</p>
        <p className="font-medium text-ink-800">{session.email}</p>
      </div>

      <div className="mt-6">
        <div className="mb-2 flex items-center justify-between">
          <h2 className="font-semibold text-ink-900">Recent Orders ({orderCount})</h2>
            ...
            {recentOrders.map((order) => (
              <li key={order.id}>
                <Link
                  ...
                  className="flex items-center justify-between px-4 py-3 text-sm hover:bg-ink-50"
                >
                  <span className="font-medium text-ink-800">{order.orderNumber}</span>
